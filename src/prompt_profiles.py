...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "Treat supplied module JSON as the source of truth and make no assumptions beyond that evidence. "
    ...
    "Clearly distinguish local-host evidence from network-observed or imported evidence about other devices. "
...
DEFAULT_USER_PROMPT = (
    ...
    "Make the potential business/mission impact clear, especially where evidence suggests phishing, credential theft, invoice fraud, donor-data exposure, service disruption, or outdated internet-facing software. "
    "Cite the evidence filename for every finding so the GUI can link the report back to JSON evidence. "
    "For every affected host or device, state whether the evidence is local to the scan-assess machine, imported inventory about another endpoint, DNS activity observed for another device, or a network device found by SafeSniff. "
    ...
    "Do not claim compromise from DNS evidence alone. "
    "It must be in the following markdown format:\n"
    "## Summary\n"
    "- A brief summary of the overall findings.\n"
    "## Findings\n"
    "- Evidence-backed observations with filenames.\n"
    "## Actionable Items\n"
...
def validate_prompt_profile(profile: PromptProfile) -> list[str]:
    ...
    required_signals = {
        "source-of-truth evidence": ["source of truth", "supplied files", "supplied module json", "only supplied"],
        "provenance awareness": ["provenance"],
        "weak-evidence caution": ["not proof", "do not prove", "do not invent", "weak"],
